	...
	public static void main(String[] args) {
		...
		try {
			...
			while(scanner.hasNextLine()) {
				...
				//If next line contains remove word then remove root node else add node to the heap
				if ("remove".equalsIgnoreCase(nextLine)) {
					remove();
				} else if (nextLine.contains("add")) {
					int newElement = Integer.parseInt(nextLine.split("add ")[1]);
					add(newElement);
				}
			}
		} catch (Exception e) {
